...
const displayVehicleNumber = computed(() => appStore.challanVerificationVehicleNumber || '')
const challanList = computed(() => appStore.challanVerificationResults || [])
const hasChallan = computed(() => challanList.value.length > 0)

</script>

<template>
  <div class="rc-result-page">
    <template v-if="!displayVehicleNumber && challanList.length === 0">
      ...
      <div class="content-wrapper">
        <div class="empty-state-banner bento-card" style="flex-direction: column; gap: 16px; text-align: center;">
          <AlertCircle :size="48" style="color: #94a3b8;" />
          <h3>No challan data found</h3>
          <p>Please go back and verify a vehicle number.</p>
          <button class="action-btn primary-btn" @click="emit('back')">Check Another Vehicle</button>
        </div>
      </div>
    </template>
    ...
    <div class="content-wrapper">
      ...
      <div class="premium-status-banner" :class="{ 'is-active': !hasChallan }">
        ...
        <div class="status-info">
          <h2>{{ hasChallan ? 'Challan Found' : 'No Pending Challan' }}</h2>
          <p>{{ hasChallan ? `Found ${challanList.length} challan(s) for your vehicle.` : 'Your vehicle is clean' }}</p>
        </div>
      </div>
          ...
          >
            ...
            <div class="card-body list-view grid-list">
              ...
              <div class="list-item-block">
                ...
                <span class="value text-block">{{ item.offense_details || item.offence_details || '-' }}</span>
              </div>
              
              <div class="card-footer-flex">
                <div class="amt-summary">
                  <span class="label">Challan Amount</span>
                  <span class="amount-val" :class="{ 'text-red': item.challan_status === 'pending' }">₹{{ item.amount != null ? parseFloat(String(item.amount)).toFixed(2) : '-' }}</span>
                </div>
