...
import { verifyRc as apiVerifyRc } from '../../../services/rc/rcApi'

const emit = defineEmits(['back', 'navigate'])
const appStore = useAppStore()
const userStore = useUserStore()

const hasRcAccess = computed(() =>
...
const handleCheckRc = async () => {
  apiError.value = ''
  const hasAccess = userStore.isTransporter ? userStore.hasTransporter499Plan : userStore.hasPremium
  if (!hasAccess) {
...
const verifyRc = async () => {
  const num = rcNumber.value.trim().toUpperCase()
  if (!num) return

  isLoading.value = true
  apiError.value = ''
  try {
    const res = (await apiVerifyRc(num)) as { status?: number | boolean; data?: Record<string, unknown>; result?: Record<string, unknown>; message?: string }
    const isSuccess = res?.status === 1 || res?.status === true
    if (isSuccess) {
      const rcData = res as Record<string, unknown>
      appStore.rcVerificationNumber = num
      appStore.rcVerificationData = rcData
      showInputModal.value = false
      rcNumber.value = ''
      emit('navigate', 'rc-check-result')
    } else {
      apiError.value = res?.message || 'Verification failed. Please try again.'
    }
  } catch (e) {
    apiError.value = e instanceof Error ? e.message : 'Something went wrong. Please try again.'
  } finally {
    ...
    <div class="web-content-container">
      ...
      <header class="page-header">
        ...
        <div class="info-grid">
          ...
          <div class="info-card hover-lift subscription-card" :class="{ 'subs-inactive': !hasRcAccess }">
            <div class="subs-left">
              <div class="card-icon" :class="hasRcAccess ? 'green-bg' : 'gray-bg'"><CheckCircle2 v-if="hasRcAccess" :size="24" /><AlertCircle v-else :size="24" /></div>
              <div class="subs-text">
                <h3>Subscription Status</h3>
                <p>{{ hasRcAccess ? 'Your account has active access to RC verification.' : 'Subscribe to access RC verification.' }}</p>
              </div>
            </div>
            <div class="subs-right">
              <div class="plan-tag" :class="hasRcAccess ? 'green-tag' : 'gray-tag'">
                <CheckCircle2 v-if="hasRcAccess" :size="18" />
                <AlertCircle v-else :size="18" />
                {{ hasRcAccess ? '₹499 Plan Active' : 'Subscription Required' }}
              </div>
              <div class="subs-warning">
                <AlertCircle :size="14" /> {{ hasRcAccess ? 'Maintain an active subscription to keep access.' : 'RC Check requires an active ₹499 plan.' }}
              </div>
            </div>
          </div>
        </div>
